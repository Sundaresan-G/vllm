...
##SBATCH --job-name=vllm_rtx5070_disagg
##SBATCH --output=slurm-rtx5070-disagg-%j.out
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# Avoid contacting the Hugging Face Hub at startup/runtime. The Hub's
# model-metadata API can return transient 504 Gateway Timeouts that stall both
# the vLLM servers and the `vllm bench serve` client for minutes. Exporting
# here makes every child process (launchers, proxy, benchmark) inherit it.
# Requires the model to already be present in the local HF cache.
...
GPU_ENV="vllm_0.25.1_xpu"
CPU_ENV="vllm_0.25.1_cpu"
...
main() {
    ...
    setsid bash -c "
        export VLLM_NIXL_SIDE_CHANNEL_PORT=5800
    ...
    setsid bash -c "
        ...
        # Disable the external NCCL network plugin (libnccl-net*.so) which
        # segfaults in ncclNetPluginInit for TP>1. Single-node TP only needs
