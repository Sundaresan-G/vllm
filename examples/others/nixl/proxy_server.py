...
async def lifespan(app: FastAPI):
    ...
    decoder_base_url = (
        f"http://{global_args.decoder_host}:{global_args.decoder_port}"
    )
...
async def start_profile(request: Request):
    ...
    try:
        ...
        # Send request to prefill service, ignore the response
        await send_request_to_service(
            app.state.prefill_client, "/start_profile", req_data
        )
        ...
        async def generate_stream():
            async for chunk in stream_service_response(
                app.state.decode_client, "/start_profile", req_data
            ):
                yield chunk
...
async def stop_profile(request: Request):
    ...
    try:
        ...
        # Send request to prefill service, ignore the response
        await send_request_to_service(
            app.state.prefill_client, "/stop_profile", req_data
        )
        ...
        async def generate_stream():
            async for chunk in stream_service_response(
                app.state.decode_client, "/stop_profile", req_data
            ):
                yield chunk
...
async def healthcheck():
    ...
    return {
        "status": "ok",
        "prefill_instances": 1 if app.state.prefill_client is not None else 0,
        "decode_instances": 1 if app.state.decode_client is not None else 0
    }
